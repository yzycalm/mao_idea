...
import { listBanner,actionScene,actionBanner,addOrUpdateBanner } from '../../api/assemblyHall/index';
...
class AssBanner extends React.Component {
        ...
        this.state = {
            tableColumns: [
                ...
                
                {
                    ...
                    render: (text,record) => {
                        return <div className="word_break">
                            {getContentBT(text,record.bannerUrl)}
                        </div>
    ...
    initData() {
        ...
        this.setState({
            ...
            listBanner(this.state.query).then((res) => {
                that.setState({
                    loading: false
                }, () => {
...
addOrUpdateBanner(row).then(res => {
    ...
    handleTitle(event) {
        if (event && event.target && event.target.value) {
            let value = event.target.value;
            let data = Object.assign({}, this.state.query, {bannerTitle: value, curPage: 1})
            this.setState({
                query: data
            }, () => {
                this.initData()
            ...
            let data = Object.assign({}, this.state.query, {bannerTitle: '', curPage: 1})
            this.setState({
                query: data
            }, () => {
                this.initData()
    ...
    render() {
        ...
        return (
            <div className="gutter-example">
                <BreadcrumbCustom first="组合会场" second="banner" />
